...
var (
	...
	Upstream = map[int]string{
		DefaultProvides: "https://223.5.5.5/resolve",
	}
)
...
// New returns a new alidns provider client
func New() *Provider {
	return &Provider{
...
func (c *Provider) ECSQuery(ctx context.Context, d dns.Domain, t dns.Type, s dns.ECS) (*dns.Response, error) {
	...
	alirr := &AliDNSResponse{}
	err = json.NewDecoder(bytes.NewBuffer(buf)).Decode(alirr)
	if err != nil {
		return nil, err
	}
	...
	rr := &dns.Response{
		...
		AD:       alirr.AD,
		CD:       alirr.CD,
		Question: []dns.Question{alirr.Question},
		Answer:   alirr.Answer,
		Provider: c.String(),
	}

	if rr.Status != 0 {
		return rr, fmt.Errorf("doh: alidns: failed response code %d", rr.Status)
	}
