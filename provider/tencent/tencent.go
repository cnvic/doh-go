...
var (
	...
	Upstream = map[int]string{
		DefaultProvides: "https://120.53.53.53/dns-query",
	}
)
...
// New returns a new tencent provider client
func New() *Provider {
	return &Provider{
...
// SetProvides set upstream provides type, tencent does NOT supported
...
func (c *Provider) ECSQuery(ctx context.Context, d dns.Domain, t dns.Type, s dns.ECS) (*dns.Response, error) {
	...
	if rr.Status != 0 {
		return rr, fmt.Errorf("doh: tencent: failed response code %d", rr.Status)
	}
