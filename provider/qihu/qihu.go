...
var (
	...
	Upstream = map[int]string{
		DefaultProvides: "https://101.198.198.198/query",
	}
)
...
// New returns a new qihu provider client
func New() *Provider {
	return &Provider{
...
// SetProvides set upstream provides type, qihu does NOT supported
...
func (c *Provider) ECSQuery(ctx context.Context, d dns.Domain, t dns.Type, s dns.ECS) (*dns.Response, error) {
	...
	if rr.Status != 0 {
		return rr, fmt.Errorf("doh: qihu: failed response code %d", rr.Status)
	}
